...
do_anonymous_page函数调用该函数来为匿名页创建anon_vma实例和anon_vma_chain实例。anon_vma_prepare会调用anon_vma_chain_link函数来建立anon_vma，anon_vma_chain，vm_area_struct三者之间的关系：
	...
	1. 将 vm_area_struct 中的 anon_vma_chain 链表加入到 anon_vma_chain 中的 same_vma 链表中。这一步是为什么呢？因为anon_vma_chain 中的 same_vma 链表存储了所有vma相同的avc，所以要把vm_area_struct中已有的avc添加到same_vma中。

	3.  // 将初始化好的 anon_vma_chain 加入到 anon_vma 管理的红黑树 rb_root中



 **page_add_new_anon_rmap**:建立起 page 到 vm_area_struct 的整条反向映射链路。
